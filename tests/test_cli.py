...
def test_images_subcommand_triggers_image_scrape(tmp_path, monkeypatch):
    ...
    def fake_scrape_images(target, todo_filename, *, idp_data):
        calls.append((target, todo_filename, idp_data))
    ...
    assert calls == [(target, "todo.txt", idp_data)]
...
def test_images_subcommand_uses_defaults(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda target, todo_filename, *, idp_data: calls.append(
            (target, todo_filename, idp_data)
        ),
    )
    runner = CliRunner()
    ...
    assert result.exit_code == 0
    assert calls == [(Path("images"), "images_todo.txt", Path("idp.data"))]
